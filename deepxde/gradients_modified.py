...
class Jacobian:
    ...
    def __call__(self, i=0, j=None,mode = "train"):
        ...
        if i not in self.J:
            if backend_name in ["tensorflow.compat.v1", "tensorflow"]:
                ...
                y = self.ys[:, i : i + 1] if self.dim_y > 1 else self.ys
                if mode == "train":#create graph 意思是是否为梯度创建计算图
                    self.J[i] = torch.autograd.grad(
                        y, self.xs, grad_outputs=torch.ones_like(y), create_graph=True
                    )[0]
                if mode == "test":
                    self.J[i] = torch.autograd.grad(
                        y, self.xs, grad_outputs=torch.ones_like(y), create_graph=False
                    )[0]
...
class Jacobians:
    ...
    def __call__(self, ys, xs, i=0, j=None,mode = "train"):
        ...
        if key not in self.Js:
            if mode == "train":
                self.Js[key] = Jacobian(ys, xs,mode = "train")
            if mode == "test":
                self.Js[key] = Jacobian(ys, xs,mode = "test")
...
def jacobian(ys, xs, i=0, j=None,mode = "train"):
    """Compute Jacobian matrix J: J[i][j] = dy_i / dx_j, where i = 0, ..., dim_y - 1 and
    j = 0, ..., dim_x - 1.
    ...
    """
    if mode == "train":
        return jacobian._Jacobians(ys, xs, mode="train",i=i, j=j)
    if mode == "test":
        return jacobian._Jacobians(ys, xs, mode="test",i=i, j=j)
...
class Hessian:
    ...
    def __init__(self, y, xs,mode="train", component=None, grad_y=None):
        if backend_name in ["tensorflow.compat.v1", "tensorflow", "pytorch", "paddle"]:
            dim_y = y.shape[1]
        elif backend_name == "jax":
            dim_y = y[0].shape[0]
        ...
        if grad_y is None:
            if mode == "train":
                grad_y = jacobian(y, xs, i=component, j=None,mode = "train")
                self.H = Jacobian(grad_y, xs,mode = "train")
            if mode == "test":
                grad_y = jacobian(y, xs, i=component, j=None,mode = "train")#这里因为要二阶导所以第一次mode = "train"
                self.H = Jacobian(grad_y, xs,mode = "test")

    def __call__(self, i=0, j=0,mode="train"):
        """Returns H[`i`][`j`]."""
        if mode == "train":
            return self.H(i, j,mode="train")
        if mode == "test":
            return self.H(i, j,mode="test")
...
class Hessians:
    ...
    def __call__(self, y, xs, mode=None,component=None, i=0, j=0, grad_y=None):
        if backend_name in ["tensorflow.compat.v1", "tensorflow"]:
        ...
        if key not in self.Hs:
            if mode == "train":
                self.Hs[key] = Hessian(y, xs, mode="train",component=component, grad_y=grad_y)
            if mode == "test":
                self.Hs[key] = Hessian(y, xs, mode="test",component=component, grad_y=grad_y)
...
def hessian(ys, xs, mode="train",component=None, i=0, j=0, grad_y=None):
    """Compute Hessian matrix H: H[i][j] = d^2y / dx_i dx_j, where i,j=0,...,dim_x-1.
    ...
    """
    if mode == "train":
        return hessian._Hessians(ys, xs,mode = "train" ,component=component, i=i, j=j, grad_y=grad_y)
    if mode == "test":
        return hessian._Hessians(ys, xs,mode = "test" ,component=component, i=i, j=j, grad_y=grad_y)
