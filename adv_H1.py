...
losshistory.to_pandas().to_csv(f"results/adv_{date}_h1.csv", index=False)
...
while len(train_vxs) < total_training_vx:
    ...
    pd_frame = pd.concat([pd.read_csv(f"results/adv_{date}_h1.csv"), pd_frame], axis = 0, ignore_index=True)
    pd_frame.to_csv(f"results/adv_{date}_h1.csv", index=False)
    dde.utils.plot_loss_history(losshistory)
    plt.show()
    
torch.save(model.state_dict(), f"results/adv_model_{date}_h1.pth")
