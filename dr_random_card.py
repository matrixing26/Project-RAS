...
for func_space, sp_name in zip(
    ...
    modelsave_path = f"results/DF/rd_{sp_name}_{date}.pth"
    csv_path = f"results/DF/rd_{sp_name}_{date}.csv"
    ...
    losshistory, train_state = model.train(iterations= iters, batch_size = batch_func(len(train_vxs)))
