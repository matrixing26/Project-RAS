...
from utils.func import arg_topk, periodic
...
def plot_test():
    ...
    mea, std = test_iresult.mean(), test_iresult.std()
    test_iresult = np.clip(test_iresult, 0, mea + 4 * std)
    colorbar = fig.colorbar(ax5.scatter(test_igrid[:, 0], test_igrid[:, 1], c = test_iresult), ax = ax5)
    colorbar.set_ticks([0, mea, mea+ std, mea+ 2*std, mea+ 3*std, mea+ 4*std])
...
space = dde.data.GRF(length_scale= ls, kernel="ExpSineSquared")
...
# %%
for _ in range(100):
    net = dde.nn.pytorch.DeepONet([101, 100, 100, 100], [5, 100, 100, 100], "gelu", "Glorot normal")
    ...
    print(train_vxs.shape, train_grid.shape, train_uxts.shape)
    model.train(iterations = 50000, batch_size= 5000, test_batch_size = 10000)
    print(f"Memory used: {torch.cuda.max_memory_allocated()/ (1024 ** 3)}Gb")
    result = model.predict((test_vxs, test_grid), operator = pde_func)
    print(result.mean(), result.std())
    plot_test()
    
    timepde = dde.data.TimePDE(geomtime, pde_func, [], num_domain=10201, train_distribution="uniform")
    eval_pts = np.linspace(0, 1, 101)[:, None]
    new_data = dde.data.PDEOperator(timepde, space, eval_pts, check_num, [0])
    (vxs, _), _, c = new_data.train_next_batch()
    xts = geomtime.uniform_points(10201)
    xts = np.tile(xts, (check_num, 1))[...,(1, 0)]
    new_test = model.predict((vxs, xts), operator = pde_func)
    new_test = new_test.reshape(check_num, 10201)
    print(new_test.shape)
    mea = new_test.mean(axis = 1)
    top_ind = arg_topk(mea, select_num)
    new_test = new_test[top_ind]
    new_test = new_test.reshape(-1)
    print(new_test.shape)
    #top_20ind = arg_topk(new_test, len(new_test) // 5)
    # print(top_20ind.shape)
    new_vxs = vxs[top_ind * 10201]
    new_grid, new_uxts = burger_solver(new_vxs)
    new_grid = new_grid.reshape(101 * 101, -1)
    new_grid = np.tile(new_grid, (select_num, 1))
    new_vxs = np.repeat(new_vxs, 10201, axis = 0)
    new_uxts = new_uxts.reshape(-1, 1)
    train_vxs = np.concatenate([train_vxs, new_vxs], axis = 0)
    train_grid = np.concatenate([train_grid, new_grid], axis = 0)
    train_uxts = np.concatenate([train_uxts, new_uxts], axis = 0)
    data = dde.data.Triple((train_vxs, train_grid), train_uxts, (test_vxs, test_grid), test_uxts)
# %%
