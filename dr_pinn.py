...
test_vxs = test_data["vxs"][:10].astype(np.float32)
test_grid = test_data["xt"].reshape(-1, 2).astype(np.float32)
test_uxts = test_data["uxts"][:10].reshape(-1, 101 * 101).astype(np.float32)
...
losshistory, train_state = model.train(iterations= 50000, batch_size = 4)
...
func_space = dde.data.GRF(1.0, length_scale = 0.05, N= 1000, interp="linear")
