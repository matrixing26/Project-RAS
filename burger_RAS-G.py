...
decay_start = ("step", 10000, 0.5)
decay_middle = ("step", 10000, 0.5)
decay_end = ("step", 10000, 0.5)
...
# Net
net = dde.nn.DeepONetCartesianProd([101, 256, 256, 256], [2, 256, 256, 256], "gelu", "Glorot normal")
...
losshistory.to_pandas().to_csv(f"results/bur_{date}_rasg.csv", index=False)
...
while len(train_vxs) < total_training_vx:
    ...
    out = model.predict((vxs, xts), aux_vars = c, operator = pde)
    ...
    pd_frame = pd.concat([pd.read_csv(f"results/bur_{date}_rasg.csv"), pd_frame], axis = 0, ignore_index=True)
    pd_frame.to_csv(f"results/bur_{date}_rasg.csv", index=False)
    dde.utils.plot_loss_history(losshistory)
    plt.show()
    
torch.save(model.state_dict(), f"results/bur_model_{date}_rasg.pth")
