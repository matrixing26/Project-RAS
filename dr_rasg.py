...
for func_space, sp_name in zip(
    ...
    modelsave_path = f"results/DF/{date}/{sp_name}_rasg.pth"
    csv_path = f"results/DF/{date}/{sp_name}_rasg.csv"
    os.makedirs(f"results/DF/{date}", exist_ok=True)
    # %%
    ...
    while len(train_vxs) < total_num:
        ...
        del vxs, topk_vxs, uxts, outs, res, auxs
